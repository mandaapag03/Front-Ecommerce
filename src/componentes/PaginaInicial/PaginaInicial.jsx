...
const ProductList = () => {
  ...
  ? 'http://localhost:5009/api/Favoritos/item/excluir'
  : 'http://localhost:5009/api/Favoritos/item/adicionar';
  ...
  const response = await fetch('http://localhost:5009/api/Favoritos/item/excluir', {
  method: 'DELETE',
  headers: {
  'Content-Type': 'application/json',
  },
...
if (token) {
  ...
  const response = await fetch('http://localhost:5009/api/Carrinho/item/adicionar', {
  method: 'POST',
  headers: {
  'Content-Type': 'application/json',
    },
  ...
  const payload = getTokenPayload(token);
  const usuarioId = payload.nameid;
  const apiUrl = `http://localhost:5009/api/Carrinho/item/excluir`;
